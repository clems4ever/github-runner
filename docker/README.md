...
For stronger isolation — a Docker daemon per job, `/dev/kvm`, a kernel a job
can break — see [the VM setup](../vm/README.md) instead.
...
Bump `ARG RUNNER_VERSION` and the checksums in `Dockerfile`
